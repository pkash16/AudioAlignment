function [sorting_fn, temp_res_fn, trim_fn] = get_path_by_imagetype(type)
    ...
    %  filename has the format: [timestamp]_tRes[tRes]_Trim[Trim].avi
    %
    %  ex: 095918_tRes150_Trim4.avi
    %
    % type: "USC" for images named using USC data collection framework
    %  filename has the format usc_disc_[date]_[time]_*_Trim[Trim].avi
    ...
    if type == "USC"
       ...
       trim_fn = @(image_path) str2num( regexprep( regexp(image_path, "Trim(.*).avi", "match", "once"), {'\D*([\d\.]+\d)[^\d]*', '[^\d\.]*'}, {'$1 ', ' '} ) );
